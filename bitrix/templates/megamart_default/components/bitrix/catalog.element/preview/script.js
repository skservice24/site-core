...
*/
		...
		initConfig: function()
		{
			...
			this.config.useLazyImages = this.params.CONFIG.USE_LAZY_IMAGES;
				...
				if (this.offers[index].SLIDER_COUNT > 0)
				{
					...
					for (i in this.offers[index].SLIDER)
					{
						if (this.offers[index].SLIDER.hasOwnProperty(i))
						{
							...
							if (i > this.sliderSlideCount - 2 && this.offers[index].SLIDER_COUNT > this.sliderSlideCount)
							{
...
*/
		...
		showDescriptionTab: function(tabPane)
		{
			...
			active = Array.from(tabBtn.parentNode.parentNode.children).filter(function(item) {
				return item.firstElementChild && item.firstElementChild.classList.contains('active');
			})[0];

			if (active)
			{
				$(active.firstElementChild).trigger('shown.bs.tab');
			}
		},

		hideDescriptionTab: function(tabPane)
		{
			var tabBtn = document.getElementById(tabPane.getAttribute('aria-labelledby')),
				active;
			...
			if (active)
			{
				$(active.firstElementChild).tab('show');
			}
		},
...
*/
		...
		buy1Click: function(event)
		{
			...
			else if (this.currentPriceMode === 'Q')
				data['QUANTITY_' + productID] = parseFloat(this.currentPrices[this.currentPriceSelected].MIN_QUANTITY);
			else
				data['QUANTITY_' + productID] = this.stepQuantity;
