;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		bindTotalEvents: function(node)
		{
			...
			BX.bind(this.getEntity(node, 'basket-buy1click-button'), 'click', BX.proxy(this.buy1clickAction, this));
		},
		...
		buy1clickAction: function(event)
		{
			...
			for (var id in this.items)
			{
				...
				{
					data['QUANTITY_' + this.items[id].PRODUCT_ID] = this.items[id].QUANTITY;
				}
			}
		...
		{
			var customInfoBlock = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-custom-info');

			if (BX.type.isDomNode(customInfoBlock))
			{
				var contentNode = this.getEntity(customInfoBlock, 'basket-custom-info-content');

				if (BX.type.isDomNode(contentNode))
				{
					var content = '';

					if (
						BX.type.isArray(this.result.CUSTOM_INFO_MESSAGES) &&
						this.result.CUSTOM_INFO_MESSAGES.length > 0
					) {

						for (var i in this.result.CUSTOM_INFO_MESSAGES)
						{
							if (this.result.CUSTOM_INFO_MESSAGES.hasOwnProperty(i))
							{
								content += this.result.CUSTOM_INFO_MESSAGES[i];
							}
						}
					}
					...
					if (content)
					{
						...
						customInfoBlock.style.display = 'block';
					}
					else
					{
						contentNode.innerHTML = '';
						customInfoBlock.style.display = 'none';
					}
				}
			}
		},
