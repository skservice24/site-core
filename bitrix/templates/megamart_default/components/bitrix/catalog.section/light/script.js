(function() {
	...
	if (!!window.JCCatalogSectionLightComponent)
		return;

	window.JCCatalogSectionLightComponent = function(params) {
	...
	window.JCCatalogSectionLightComponent.prototype =
	{
...
*/
		...
		processItems: function(itemsHtml, position)
		{
			...
			if (items.length)
			{
				...
				if (RS !== undefined)
				{
					RS.Init(['popup', 'lazy-images'], this.container);
				}
			}
				...
				if (header && header.getAttribute('data-showed') != 'true')
				{
					...
					if (animate)
					{
						new BX.easing({
							...
							transition: BX.easing.makeEaseOut(BX.easing.transitions.quad),
							step: function(state){
								header.firstElementChild.style.opacity = state.opacity / 100;
							},
							complete: function(){
								header.firstElementChild.removeAttribute('style');
					...
					{
						header.firstElementChild.style.opacity = 100;
					}
				}
			}
		},
