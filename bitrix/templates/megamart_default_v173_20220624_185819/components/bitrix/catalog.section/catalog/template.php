...
{
	$sectionAttr .= ' data-spy="item" data-target="#l-main__nav" data-title="'.$arParams['RS_LIST_SECTION_TITLE'].'"';
}
...
<div class="catalog-section" data-entity="<?=$containerName?>">
	...
	if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS']))
	{
		...
		else
		{
			$collapsed = false;
			$collapsedRowIndex = 0;
			if ($arParams['PRODUCT_ROWS_USE_COLLAPSE'] == 'Y')
			{
...
<script>
	...
	var <?=$obName?> = new JCCatalogSectionComponent({
		...
		useCollapse: <?=$arParams['PRODUCT_ROWS_USE_COLLAPSE'] == 'Y' ? 'true' : 'false'?>,
